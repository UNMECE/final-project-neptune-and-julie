...
Region::Region (std::string name, double waterLevel, double waterNeed, double waterCapacity) 
   : name(name), waterLevel(waterLevel), waterNeed(waterNeed), waterCapacity(waterCapacity),
     isFlooded(false), isInDrought(false), overflow(0), drought(0) 
{
...
AcequiaManager::AcequiaManager() : hour(0), SimulationMax(100), isSolved(false), solvedTime(0) {}
...
void AcequiaManager::loadRandomParameters() {
   std::ifstream infile("RandomValues.dat");
   if (!infile.is_open()) {
      std::cerr << "Error: Could not open RandomValues.dat, using default values" << std::endl;
      initializeRegions();
      SimulationMax = 100;
      return;
   }
   ...
   // Read "Max Simulation Time" line
   std::getline(infile, line);
   // Read the max time value
   std::getline(infile, line);
   SimulationMax = std::stoi(line);
   std::cout << "Loaded SimulationMax: " << SimulationMax << std::endl;

   // Skip "Random Values" line
   std::getline(infile, line);
   ...
   for (int i = 0; i < 3; i++) {
      ...
      std::string name;
      double waterLevel, waterNeed, waterCapacity;
      char comma;

      std::getline(ss, name, ',');
      ss >> waterLevel >> comma >> waterNeed >> comma >> waterCapacity;

      std::cout << "Loading " << name << ": Level=" << waterLevel
                << ", Need=" << waterNeed << ", Capacity=" << waterCapacity << std::endl;

      regions.push_back(new Region(name, waterLevel, waterNeed, waterCapacity, false, false, 0, 0));
   }
...
void AcequiaManager::initializeRegions() {
   ...
   // Create regions with proper initialization
   regions.push_back(new Region("North", 53.0, 49.5, 55, false, false, 0, 0));
   regions.push_back(new Region("South", 40.0, 39.0, 45, false, false, 0, 0));
   regions.push_back(new Region("East", 25.0, 26.0, 50, false, false, 0, 0));
}

void AcequiaManager::initializeCanals(){
   // Clear existing canals
   for (auto canal : canals) delete canal;
   canals.clear();
...
//showing the current state of each region
...
void AcequiaManager::evaluateSolution(){
   ...
   int penalty = penalties();
   score -= penalty;

   std::cout << "\n=== Score Breakdown ===" << std::endl;
   for(const auto& region: regions){
      std::cout << region->name << ": " << (!region->isFlooded && !region->isInDrought && region->waterLevel >= region->waterNeed ? "10 points" : "0 points") << std::endl;
   }
   std::cout << "Penalties: -" << penalty << " points" << std::endl;

   if(isSolved) {
      score += 50;
      std::cout << "Bonus: +50 points (solved in " << solvedTime << " hours)" << std::endl;
